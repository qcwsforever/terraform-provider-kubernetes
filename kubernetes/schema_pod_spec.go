	...
	v["empty_dir"] = &schema.Schema{
		...
		Elem: &schema.Resource{
			Schema: map[string]*schema.Schema{
				"medium": {
					Type:         schema.TypeString,
					Description:  `What type of storage medium should back this directory. The default is "" which means to use the node's default medium. Must be an empty string (default) or Memory or HugePages-2Mi or HugePages-1Gi. More info: https://kubernetes.io/docs/concepts/storage/volumes#emptydir`,
					Optional:     true,
					Default:      "",
					ForceNew:     !isUpdatable,
					ValidateFunc: validateAttributeValueIsIn([]string{"", "Memory", "HugePages-2Mi", "HugePages-1Gi"}),
				},
				"size_limit": {
					Type:             schema.TypeString,
